...
def vis_distance(keyword,sstring):
        ...
        for index in range(len(sstring)):
                if keyword in sstring[index]:
                        flag = '米'#要不要把公里变成米
                        if '米每秒' in sstring[index]:
                                sstring[index]=sstring[index].replace('米每秒','米/秒')
...
def wind_direction_speed(sstring):
        direction = wind_direction(sstring)
        if direction =='null':
                return 'null'
        direction=direction+'风'
        answer1 = vis_distance(direction,sstring)
        if answer1!=[]:
                return answer1
        if answer1 == []:
                match_location1 = re.search(direction,sstring).span()
                begin = match_location1[1]+1
                i = begin
                while sstring[i]!='，' and sstring[i]!=',':
                        i = i+1
                if i >len(sstring)-1:
                        i = len(sstring)-1
               
                answer2 = vis_distance('风速',sstring[begin:i])
                if answer2!=[]:
                        return answer2
                if answer2==[]:
                        begin = match_location1[0]-1
                        if sstring[begin]=='的':
                                i = begin
                                while i>0 and sstring[i]!='，' and sstring[i]!=',':
                                        i = i-1
                                if i<0:
                                        i = 0
                                answer3 = vis_distance('出现',sstring[i:match_location1[1]])
                                if answer3!=[]:
                                        return answer3
...
def wind(sstring):
        ...
        dict_wind['方向风速']=wind_direction_speed(sstring)
